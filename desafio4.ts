...
let apiField = document.getElementById('api-key') as HTMLInputElement;
...
if(searchButton) {
  ...
  searchButton.addEventListener('click', async () => {
    ...
    if(searchField) {
      ...
      let listaDeFilmes: any = await procurarFilme(searchField.value);
...
class HttpClient {
  ...
  static async get(options: OptionsRequest) {
...
async function procurarFilme(query: string) {
  ...
  let result = await HttpClient.get(options)
  return result
}
...
async function criarRequestToken () {
  ...
  let result: any = await HttpClient.get(options)
